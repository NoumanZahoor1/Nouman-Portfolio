...
const BlogManager = ({ blogs, onAdd, onEdit, onDelete, onTogglePublish }) => {
  return (
    <div className="bg-white dark:bg-gray-800 rounded-[3rem] shadow-xl border border-white/5 overflow-hidden">
      <div className="p-8 border-b border-gray-100 dark:border-white/5 flex justify-between items-center bg-gray-50/50 dark:bg-white/5">
        <h2 className="text-xl font-black uppercase tracking-widest text-neutral-900 dark:text-white leading-none">Journal Entries</h2>
        <button
          onClick={onAdd}
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead className="bg-gray-50/50 dark:bg-white/5">
            <tr>
              <th className="px-8 py-5 text-left text-[10px] font-black uppercase tracking-[0.2em] text-gray-500">Post Title</th>
              <th className="px-8 py-5 text-left text-[10px] font-black uppercase tracking-[0.2em] text-gray-500">Category</th>
              <th className="px-8 py-5 text-left text-[10px] font-black uppercase tracking-[0.2em] text-gray-500">Visibility</th>
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-white/5">
            {blogs.map((blog) => (
              <tr key={blog._id} className="hover:bg-gray-50/50 dark:hover:bg-white/1 transition-colors group">
                <td className="px-8 py-6">
                  <p className="font-black text-neutral-900 dark:text-white uppercase tracking-widest leading-none mb-2">{blog.title}</p>
                  <p className="text-[10px] text-gray-500 uppercase tracking-widest font-bold font-mono">{blog.slug}</p>
                </td>
                <td className="px-8 py-6">
                ...
                <td className="px-8 py-6">
                  ...
                  >
                    {blog.published ? <FiEye size={12} /> : <FiEyeOff size={12} />}
